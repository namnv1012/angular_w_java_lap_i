...
public class UserMapper {
    ...
    public List<AdminDto> usersToAdminUserDTOs(List<User> users) {
        return users.stream().filter(Objects::nonNull).map(this::userToAdminUserDTO).collect(Collectors.toList());
    }

    public AdminDto userToAdminUserDTO(User user) {
        return new AdminDto(user);
    }

    public List<User> userDTOsToUsers(List<AdminDto> userDTOs) {
        return userDTOs.stream().filter(Objects::nonNull).map(this::userDTOToUser).collect(Collectors.toList());
    }

    public User userDTOToUser(AdminDto userDTO) {
        if (userDTO == null) {
            return null;
        } else {
            User user = new User();
